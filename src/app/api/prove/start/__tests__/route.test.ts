...
function makeRequest(body: unknown) {
  return new Request("http://localhost/api/prove/start", {
    method: "POST",
...
describe("POST /api/prove/start", () => {
  it("returns 401 when not authenticated", async () => {
    mockAuth(null);

    const req = makeRequest({ idea: "test idea" });
  ...
  it("returns 400 on invalid body (missing idea)", async () => {
    mockAuth({ id: "user-1" });
  ...
  it("returns 200 with session id on valid request", async () => {
    ...
    const req = makeRequest({ idea: "A marketplace for local artisans" });
    const res = await POST(req);
  ...
  it("returns 500 when database insert fails", async () => {
    ...
    const body = await res.json();
    expect(body.error).toBe("Failed to create prove session");
  });
});
